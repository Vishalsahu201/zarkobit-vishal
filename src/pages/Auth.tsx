...
import { Crown, Eye, EyeOff, Mail, Lock, User, Phone, Store, Scissors } from "lucide-react";
...
const Auth = () => {
  ...
  const [role, setRole] = useState<"user" | "salon_owner">("user");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Update profile role
        if (data.user) {
          await supabase.from("profiles").update({ role, phone: form.phone }).eq("user_id", data.user.id);
        }
          ...
          <div className="mb-4 flex gap-3">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-medium transition-all ${
                role === "user" ? "bg-primary text-primary-foreground gold-glow" : "bg-secondary text-foreground"
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-medium transition-all ${
                role === "salon_owner" ? "bg-primary text-primary-foreground gold-glow" : "bg-secondary text-foreground"
              }`}
            >
              <Store className="h-4 w-4" /> Salon Owner
            </button>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl bg-card p-6 gold-border">
          {!isLogin && (
            <div className="relative">
              ...
              <input type="text" placeholder={role === "salon_owner" ? "Owner Name" : "Full Name"} required value={form.name}
          ...
          <div className="relative">
            ...
            <input type={showPassword ? "text" : "password"} placeholder="Password" required minLength={6} value={form.password}
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? "Please wait..." : isLogin ? "Sign In" : `Create ${role === "salon_owner" ? "Salon" : ""} Account`}
          </button>
